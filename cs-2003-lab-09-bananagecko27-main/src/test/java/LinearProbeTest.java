...
public class LinearProbeTest {
  ...
  /** Returns the private field table of a {@code LinearProbe}. */
  private <K, V> Object[] getTable(LinearProbe<K, V> hashTable) {
    try {
      ...
      throw new NoSuchFieldError(
          "expected that LinearProbe has field (variable) Object[] named table");
    }
  }

  @Test
  @TestGroup("default")
  @DisplayName("LinearProbe(int, double) should construct an empty table with the input capacity")
  @Deps("size()")
  public void testConstructorConstructsAnEmptyTable() {
    LinearProbe<String, String> table = new LinearProbe<>(11, 1.0);
  ...
  public void testContainsReturnsTrueIfKeyIsInTableDespiteCollisions() {
    LinearProbe<Integer, String> table = new LinearProbe<>(11, 1.0);
    table.put(0, "v1");
    table.put(11, "v2");
    table.put(22, "v3");
    table.put(33, "v4");
  ...
  public void testContainsReturnsFalseIfKeyIsNotInTable() {
    assertThat(new LinearProbe<String, String>(11, 1.0).contains("key"), is(false));
  }
  ...
  public void testGetReturnsNullIfKeyIsNotInTable() {
    assertThat(new LinearProbe<String, String>(11, 1.0).get("key"), is(nullValue()));
  }
  ...
  @Test
  @TestGroup("put")
  @DisplayName("put(K, V) should store the key-value entry in the table using linear probing")
  public void testPutStoresEntryIntoTableUsingLinearProbing() {
    LinearProbe<Integer, String> table = new LinearProbe<>(11, 1.0);
    ...
    assertThat(table.put(22, "v3"), is(equalTo(4))); // index 0, 1, 2, 3
    assertThat(table.put(3, "test-v3"), is(equalTo(2))); // index 3, 4
    assertThat(table.put(33, "v4"), is(equalTo(6))); // index 0, 1, 2, 3, 4, 5
    assertThat(table.put(5, "test-v4"), is(equalTo(2))); // index 5, 6
    assertThat(table.put(44, "v5"), is(equalTo(8))); // index 0, 1, 2, 3, 4, 5, 6, 7
    assertThat(table.put(7, "test-v5"), is(equalTo(2))); // index 7, 8
  }

  @Test
  @TestGroup("put")
  @DisplayName(
      "put(K, V) should be able to wrap around to the beginning of the table when using linear"
          + " probing")
  public void testPutWrapsAroundTableWithLinearProbing() {
    LinearProbe<Integer, String> table = new LinearProbe<>(11, 1.0);
    assumeThat(table.put(9, "v1"), is(equalTo(1)));
    assumeThat(table.put(10, "v2"), is(equalTo(1)));
    assumeThat(table.put(0, "v3"), is(equalTo(1)));
    assertThat(table.put(20, "v4"), is(equalTo(4)));
    assertThat(table.put(1, "test-v4"), is(equalTo(2)));
  }
  ...
  public void testPutRehashesTableUponExceedingLoadFactor() {
    LinearProbe<Integer, String> table = new LinearProbe<>(11, 0.33);
